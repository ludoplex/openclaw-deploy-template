...
## ⚠️ CORRECTION NOTICE (Added 2026-02-07)
The architecture below describes the **llamafile pattern** which is WRONG for Sokol GUI.
For Sokol GUI, use the **cosmo-sokol pattern** (bullno1/cosmo-sokol) instead:
- ALL backends compiled INTO the APE with prefixes
- Runtime dispatch shim, NO separate helper DSOs
- See `MEMORY.md` and `skills/sokol-cosmo/SKILL.md` for correct pattern

The llamafile pattern (extract DSO from PKZIP) is only for GPU compute (CUDA/Metal), NOT for Sokol GUI.
...
### ~~The Correct Architecture (llamafile pattern)~~ — SUPERSEDED
**NOTE: This section describes the llamafile pattern which is WRONG for Sokol GUI. See correction above.**
...
- **BUT: llamafile pattern is for GPU compute, NOT Sokol GUI!**
